...
function check(e) {
  ...
  /*
   * Заметка о клиенте — то же самое, но подпись в ней значит больше.
   *
   * По payload.trainer_id решается, кто заметку прочитает: правило доступа
   * пускает к ней только автора (NOTE_AUTHOR в schema.mjs). Значит подпись —
   * это не украшение, а адрес доставки, и без сверки тренер заводит заметку о
   * своём клиенте с чужим trainer_id. Достаётся она следующему тренеру этого
   * же клиента: тот открывает карточку и читает про человека слова, которых
   * не писал, за подписью, которая выглядит как его собственная.
   *
   * Правка — это дописывание своей же заметки, поэтому у неё сверяется не
   * подпись с пришедшим, а подпись с тем, что уже лежит: сменить автора у
   * существующей строки нельзя ни автору, ни кому-то ещё.
   */
  if (tbl === 'trainerNotes' && e.auth) {
    const was = before ? payloadOf(before) : null

    if (was) {
      if (payload.trainer_id && was.trainer_id && payload.trainer_id !== was.trainer_id) {
        throw new BadRequestError('Автор заметки не меняется')
      }
    } else if (payload.trainer_id && payload.trainer_id !== e.auth.id) {
      throw new BadRequestError('Заметку можно завести только от своего имени')
    }
  }
