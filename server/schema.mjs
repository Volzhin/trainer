 ...
 *
 * Откуда 2 ГБ. Телефон снимает 1080p60 примерно по 130 МБ на минуту, 4K60 —
 * около 400 МБ; два гигабайта это пять минут съёмки в худшем случае и час с
 * лишним в обычном. Подход длиннее пяти минут не бывает, а промахнуться
 * человек может — снял тренировку целиком, — и тогда лучше принять файл, чем
 * объяснять ему, что его работа пропала: переснять подход нельзя.
 *
 * Прежние 64 МБ ставились под клиентскую проверку в 60 МБ, а её больше нет:
 * ролик прикрепляется любого веса и живёт на устройстве в любом случае
 * (см. addAttachment в src/db/coach.ts). Здесь решается только то, доедет ли
 * он до тренера.
 *
