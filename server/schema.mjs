 ...
 *
 * Тумбстоун — исключение из исключения. Удаление приезжает без содержимого
 * (payload: null, см. drainDeletes в src/db/sync.ts), автора в нём нет, и без
 * этой поблажки второй телефон тренера никогда не узнал бы, что заметка
 * стёрта, — она осталась бы там навсегда. Наружу при этом уходит только факт
 * «заметка была и удалена тогда-то»: ни текста, ни автора в тумбстоуне нет.
 * Стереть чужую заметку это не помогает — правка судится по строке ДО
 * изменения, а её не видно.
 */
const NOTE_AUTHOR =
  '(tbl != "trainerNotes" || payload.trainer_id = @request.auth.id || deleted = true)'
