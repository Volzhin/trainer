 ...
 *
 * Откуда 300 МБ. С тех пор как ролик сжимается на самом устройстве
 * (см. `src/lib/video.ts`), сорокапятисекундный подход весит около 8 МБ, а
 * не 92 МБ, и в этот предел укладывается с тридцатикратным запасом. Запас
 * нужен двум случаям: устройству, где сжатие не сработало (старый браузер,
 * отказ кодека — тогда уезжает оригинал), и роликам, снятым до появления
 * сжатия: они лежат на телефонах несжатыми и уедут, как только человек
 * нажмёт «Отправить ещё раз».
 *
 * Почему не 2 ГБ, как было недолгое время. Столько никто не отправляет
 * осмысленно: это промах — снял тренировку целиком вместо подхода. Принять
 * такой файл значит положить гигабайт на диск, который делится с чужим
 * проектом, ради ролика, который никто не станет смотреть. Отказ виден
 * человеку словами и с кнопкой повтора, а место на диске — нет.
 *
 * ВЫШЕ ЭТОГО ЧИСЛА ПОДНИМАТЬ БЕССМЫСЛЕННО, пока не поднят nginx: он стоит
 * перед бэкендом и отбивает тело раньше — `client_max_body_size` должен быть
 * не меньше, см. «Загрузка тяжёлых файлов» в DEPLOY.md. И помните про диск:
 * файлы лежат в /opt/trainer/pb/pb_data, на одной машине с чужим проектом.
 */
...
async function ensure(token, byName, def) {
  ...
  //
  // Существующие поля не пропускаем, а накладываем описание поверх того, что
  // на сервере. Раньше поле, заведённое однажды, больше не менялось: скрипт
  // досылал только отсутствующие. Значит правка настройки поля — например
  // maxSize у файла — не доезжала никуда, а в репозитории выглядела
  // применённой; поправить её можно было только кликами в админке, то есть
  // ровно тем способом, который здесь запрещён. Идентификатор поля берём с
  // сервера: без него PocketBase считает поле новым и отбивает по имени.
  const have = new Map(current.fields.map((f) => [f.name, f]))
  const wanted = new Map(def.fields.map((f) => [f.name, f]))
  const merged = [
    ...current.fields.map((f) => ({ ...f, ...wanted.get(f.name) })),
    ...def.fields.filter((f) => !have.has(f.name)),
  ]
