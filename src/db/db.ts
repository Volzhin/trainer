...
export interface WorkoutTemplateItem {
  ...
  rest_seconds: number
  /**
   * Слово тренера про это упражнение в этой программе: «пауза внизу»,
   * «последний подход до отказа», «вес не поднимаем, следим за коленом».
   *
   * Живёт в строке шаблона, а не в feedback: тот привязан к тренировке,
   * которая уже случилась, и до зала не доезжает. Указание же нужно человеку
   * ровно в тот момент, когда он стоит у снаряда, — и одинаково при каждом
   * повторении дня, а не однажды после разбора.
   */
  note?: string
...
export interface UserProfile {
  ...
  /**
   * Отсчёт между подходами. Необязательное намеренно: «не задано» значит
   * «включено», как было до появления настройки, — тогда не нужна ни новая
   * версия Dexie ради одного поля, ни дописывание умолчания во все места,
   * где профиль заводится (регистрация, сид, обмен, карточка клиента).
   * Выключает отдых целиком, включая длительность из шаблона программы:
   * человек, которому таймер мешает, отключает его один раз, а не в каждой
   * программе отдельно.
   */
  rest_timer_enabled?: 0 | 1
  haptics_enabled: 0 | 1
  sound_enabled: 0 | 1
  /**
...
export interface AppState {
  ...
  lang?: Lang
  /**
   * Достижениеи, о которых человеку уже сказали, — по каждому аккаунту отдельно.
   *
   * Сам достижение нигде не хранится: он выводится из тренировок, замеров и
   * заданий (см. `db/game.ts`). Хранить нужно единственное — поздравили или
   * ещё нет. Это про экран, а не про работу, поэтому лежит в настройках
   * устройства и в обмен не идёт.
   *
   * По аккаунту, а не одной кучей: на общем телефоне тренер и его же
   * тестовый клиент — разные люди со своими достижениеами, и общий список гасил
   * бы поздравления одного отметками другого.
   */
  seen_achievements?: Record<string, string[]>
  /**
 ...
 */
/**
 * Что разбирает тренер. Задание сюда попало не ради разбора — принятое
 * задание отмечать больше нечем: сама отметка обязана лежать в строке
 * тренера, иначе правка клиента затрёт её при обмене.
 */
export type ReviewTarget = 'workout' | 'nutrition' | 'task'
...
export interface ClientTask {
  ...
  /**
   * Чем задание закрыто: id записи, которая его и выполнила, — замера или
   * разобранного отчёта InBody.
   *
   * Ссылкой, а не копией цифр: тренер должен смотреть на ту же строку, что
   * лежит в истории замеров, иначе исправленная опечатка в весе разошлась бы
   * с тем, что он читает в задании. У фото ссылки нет — они и так найдутся
   * по task_id во вложениях.
   */
  result_ref?: string
  completed_at?: number
...
export interface Attachment {
  ...
  task_id?: string
  /**
   * День съёмки серии фото прогресса (YYYY-MM-DD).
   *
   * Раньше снимок тела опознавался заданием, и это была ошибка: задание
   * одноразовое, а сравнивают снимки через месяцы. Второй серии деться было
   * некуда — её просто негде было завести. Дата же есть у любой съёмки, в
   * том числе у сделанной без всякого задания.
   *
   * Не индексируем намеренно: индекс потребовал бы новой версии Dexie, а
   * выборка идёт по `user_id`, который уже проиндексирован, — фото тела у
   * одного человека десятки, не тысячи. У старых снимков поля нет вовсе,
   * и день им считается по `created_at` (см. `db/photos.ts`).
   */
  shot_date?: string
  /**
  ...
  remote_file?: string
  /**
   * Почему файла до сих пор нет на сервере. Пусто — очередь просто до него не
   * дошла или связи нет; это обычное состояние офлайн-первого приложения, и
   * выгрузка вернётся к нему сама.
   *
   * `too_big` ставит выгрузка, когда сервер отказал именно по размеру
   * (413 от nginx или maxSize от PocketBase). Повторять такую отправку
   * бессмысленно: файл не похудеет, а каждый проход обмена тратил бы на него
   * мобильный трафик и упирался в тот же отказ. Помеченное выгрузка
   * пропускает, а экран говорит человеку, что ролик остался на устройстве, —
   * молчание здесь хуже отказа.
   *
   * Поле не индексируется, поэтому новой версии Dexie не требует: схема
   * хранилища не меняется, старые строки просто читаются без него.
   */
  upload_error?: 'too_big'
  created_at: number
  updated_at: number
}
