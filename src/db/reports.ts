...
const REQUIRED_TASKS: {
  ...
  },
  {
    ...
    title: 'Зачем мне это',
    description:
      'Опиши подробно свою ситуацию сейчас. Как себя чувствуешь? Эмоции? Общее ' +
      'состояние? Уровень энергии? Уровень удовлетворения от текущего состояния? ' +
      'Дальше ответь на вопрос: зачем я хочу изменить тело и привычки и как изменится ' +
      'моя жизнь, когда получится? Сохрани и отправь нам — будем возвращаться к этому ' +
      'в трудные моменты.',
    due: true,
  },
  {
...
/**
 * Сколько файлов приложено к заданию.
 *
 * Спрашивают один раз, при открытии подтверждения, а не подпиской: индекса по
 * task_id нет, поэтому Dexie перебирает таблицу вложений целиком — вместе с
 * видео внутри. Переигрывать это на каждое изменение вложений значило бы
 * вычитывать в память все ролики на устройстве.
 */
export async function taskFileCount(taskId: string): Promise<number> {
  return db.attachments.filter((a) => a.task_id === taskId).count()
}

/**
 * Тренер снимает выданное задание.
 *
 * Через deleteSynced, а не прямым delete: строка задания принадлежит клиенту
 * и уже уехала на сервер — стёртая только здесь, она вернулась бы первым же
 * приёмом данных, и клиент продолжал бы видеть задание, которое тренер снял.
 *
 * Отметка о приёме уходит вместе с заданием: она живёт отдельной строкой
 * тренера и без задания превращается в ссылку в пустоту.
 *
 * А вот приложенные файлы остаются, и это осознанно. Сносить их вместе с
 * заданием казалось опрятнее — не остаётся недостижимых файлов, — но фото
 * «до/после» лежат единственной копией на телефоне клиента, и удаление
 * доехало бы до него: тренер, снявший своё же задание, стирал бы чужие
 * снимки. Недостижимый файл — цена меньшая, чем потерянный, и человеку об
 * этом говорят до нажатия, а не после.
 */
export async function deleteTask(taskId: string, trainerId = currentUserId()) {
  const task = await db.tasks.get(taskId)
  if (!task) return

  // Ключ отметки выводится из цели — тот же, что ставит markReportReviewed.
  await deleteSynced('reviews', `${trainerId}:${task.client_id}:task:${taskId}`)
