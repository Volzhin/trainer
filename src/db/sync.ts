...
const TRAINER_AUTHORED: readonly string[] = [
  ...
  // этих двух его слова остались бы на его же телефоне. Замеров здесь нет
  // намеренно: их тренер не комментирует, они просто идут в статистику.
  'workoutReports',
  'nutritionDays',
]
  ...
  for (const a of pending) {
    ...
    try {
      ...
      const res = await uploadAttachment({
        ...
        note: a.exercise_id ?? a.nutrition_date ?? '',
        file: a.blob,
        filename: `${a.id}.${ext}`,
      })
