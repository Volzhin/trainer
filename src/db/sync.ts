  ...
  for (const item of deletes) {
    ...
    try {
      ...
      await upsertRecord({
        ...
        deleted: true,
        /*
         * Удаление уезжает пустым — в нём нет ни текста, ни дат, только факт
         * «строка была и её больше нет».
         *
         * Единственное исключение — заметка тренера: доступ к ней даёт подпись
         * автора, и тумбстоун без подписи не проходил бы правило вовсе. Раньше
         * ради этого в правиле стояла поблажка «удалённое пускаем без автора»,
         * а она открывала факт заметки любому, кто на этот момент числится
         * тренером клиента. Дешевле донести автора, чем держать дыру в правиле.
         */
        payload:
          table === 'trainerNotes' && typeof stale.trainer_id === 'string'
            ? { trainer_id: stale.trainer_id }
            : null,
      })
      done.push(item.id)
    } catch {
