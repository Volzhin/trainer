...
const REMINDERS: { kind: NotificationKind; title: string; sub: string; onlineOnly?: true }[] = [
  { kind: 'weight', title: 'Внести вес', sub: 'Каждый день в 08:00' },
  {
    kind: 'measurements',
    title: 'Внести замеры',
    sub: 'Воскресенье 20:00 и понедельник 08:00',
  },
...
export function Settings() {
  ...
  const resetAll = async () => {
    ...
    // Владельца подходов передаём явно: тренировок к моменту выгрузки уже не
    // будет, а выводится он из них.
  ...
  const reseed = async () => {
    ...
    // Именно seedCatalog, а не seedIfEmpty: последний считает базу
    // заполненной, пока в ней есть хоть одна строка, — и при единственном
    // своём упражнении молча не восстановил бы ничего, оставив человека
    // вовсе без справочника.
    await seedCatalog()
    invalidateExercises()
    ...
    <div className="screen">
      <div className="header">
        ...
        <div className="grow">
          ...
          <div className="sub">
            {isTrainer ? t('Кабинет тренера') : t('Приложение и данные')}
          </div>
        </div>
      </div>
