...
import { attachmentUrl, type TrainerDoc } from '../lib/backend'
...
export function ConsentStep({
  ...
  return (
    <div className="stack">
      <div className="muted">
        {documents.length
          ? t('Чтобы начать работу с тренером, примите его документы. Нажмите на название, чтобы открыть.')
          : t('Тренер не приложил документов — подписывать нечего.')}
      </div>
        ...
        {documents.map((doc) => (
          <div className="group-row" key={doc.kind}>
            ...
            <a
              ...
              href={attachmentUrl(doc.id, doc.file)}
              target="_blank"
              rel="noreferrer"
            >
