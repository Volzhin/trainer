    ...
    <>
      ...
      <div className="card">
        <div className="mute-sm mb-3">
          {t('Клиент подписывает их при подключении. Не прикреплённое не показывается и не подписывается.')}
        </div>
            ...
            return (
              <div className="group-row" key={doc.kind}>
                <span className="grow">
                  ...
                  <span className="sub">
                    {file
                      ? `${t('прикреплён')} ${formatDate(file.created_at)}`
                      : t(doc.sub)}
                  </span>
                </span>
                ...
                {file ? (
                  <>
                    ...
                    {file.remote_id && file.remote_file && (
                      <a
                        className="btn sm"
                        href={attachmentUrl(file.remote_id, file.remote_file)}
                        target="_blank"
                        rel="noreferrer"
                      >
                        {t('Открыть')}
